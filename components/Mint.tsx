    ...
    if (window.ethereum) {
      ...
      if (currentNetwork != "242") {
        try {
          ...
          if (err instanceof Error) {
            setIsMinting(false);
            props.setStatus({
              message: `Please switch to KardiaChain.`,
              type: "error",
            });
          }
        }
      ...
      try {
        const tx = await contract.methods
          .mint(window.ethereum.selectedAddress, _amount)
          .send({ from: window.ethereum.selectedAddress, value: totalCost });
        await tx;
    ...
    <div className="mint__control">
      <div className="flex gap-2">
        <input
          ...
          id="qty"
          placeholder="Qty"
          className="border text-sm rounded-lg block w-full p-2.5 placeholder-gray-400 text-black focus:ring-[#e51d38] focus:border-[#e51d38]"
          min="1"
          max="3"
        ...
        <button
          onClick={mint}
          className="w-full flex justify-center items-center bg-[#44912d] hover:bg-[sky-700] text-white font-bold py-2 px-4 rounded-lg inline-flex"
        >
