...
const Holdings = () => {
  ...
  const fetchNFTs = async () => {
    ...
    for (let i = 0; i < balance; i++) {
      ...
      tokenMetadataURI = `https://purple-given-reindeer-860.mypinata.cloud/ipfs/${
      ...
      tokenImage = `https://purple-given-reindeer-860.mypinata.cloud/ipfs/${
        tokenImage.split("ipfs://")[1]
      }`;
      const tokenName = tokenMetadata.name;

      rawdata = await fetch(
        "https://purple-given-reindeer-860.mypinata.cloud/ipfs/QmbtUP6P16E93jf66JTTspb62UsJPBVq6T6r84nMXGx6oV?_gl=1*1ejikdi*_ga*MTUyMzc1NTk3Mi4xNjcwMzcyNzA2*_ga_5RMPXG14TE*MTY3NjAzMTgxOC4yMi4xLjE2NzYwMzE4NzkuNjAuMC4w"
      ).then((response) => response.json());

      const nft = rawdata.find(
  ...
  const onSaleNfts = async () => {
    ...
    onSaleTokens = await marketplace.methods
      .getListedNFTsByUser(userAddress)
      .call();
